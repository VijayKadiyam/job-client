    ...
    <v-layout align-center justify-center>
      ...
      <v-flex xs12 sm6 md5 px-2>
        <v-select
          ...
          v-model.number="form.subject_chapter_topic_id"
          :items="subjects"
          label="Sub Topic"
        ></v-select>
      </v-flex>
...
export default {
  ...
  async asyncData({$axios, params}) {
    ...
    let question_meta = await $axios.get(`/question_metas/${params.id}`)
  ...
  mounted() {
    this.form.question_type_id = parseInt(this.form.question_type_id)
    this.form.subject_id = parseInt(this.form.subject_id)
    this.form.subject_chapter_id = parseInt(this.form.subject_chapter_id)
    this.form.difficulty_level_id = parseInt(this.form.difficulty_level_id)
  },
  components: {
  ...
  methods: {
    async updateQuestion() {
      await this.$axios.patch(`question_metas/${this.$route.params.id}`, this.form);
