      ...
      <v-flex xs12 sm8 md6>
        <v-card class="elevation-12">
          <v-toolbar :dark="darkStatus" :height="baseHeight" :color="baseColor">
            <v-toolbar-title>Create Affiliation</v-toolbar-title>
          </v-toolbar>
          <v-card-text>
            <v-form>
              <v-text-field 
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn :dark="darkStatus" @click="store" :color="baseColor">Create Affiliation</v-btn>
          </v-card-actions>
        </v-card>
      </v-flex>
    </v-layout>
...
export default {
  ...
  methods: {
    async store() {
      await this.$axios.post(`/affiliations`, this.form)
