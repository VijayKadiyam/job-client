    ...
    <v-layout align-center justify-center>
      <v-flex xs12 sm12 md12>
        <v-card class="elevation-12">
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn :dark="darkStatus" @click="store" :color="baseColor">Update Unit Details</v-btn>
          </v-card-actions>
        </v-card>
      </v-flex>
    </v-layout>
...
export default {
  ...
  async asyncData({$axios, params}) {
    let unit = await $axios.get(`units/${params.id}`)
    return {
      form: unit.data.data
    }
  },
  ...
  methods: {
    async store() {
      await this.$axios.patch(`units/${this.$route.params.id}`, this.form)
