    ...
    <v-layout align-center justify-center>
      <v-flex xs12 sm8 md6>
        <v-card class="elevation-12">
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn :dark="darkStatus" @click="store" :color="baseColor">Update Holiday</v-btn>
          </v-card-actions>
        </v-card>
      </v-flex>
    </v-layout>
...
export default {
  ...
  async asyncData({$axios, params}) {
    let holiday = await $axios.get(`/holidays/${params.id}`)
    return {
      form: holiday.data.data
    }
  },
  data: () => ({
    holidayMenu: false,
  ...
  methods: {
    async store() {
      await this.$axios.patch(`/holidays/${this.$route.params.id}`, this.form)
